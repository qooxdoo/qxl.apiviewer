...
qx.Class.define("apiviewer.dao.Method",
{
  ...
  construct : function(meta, clazz, name) {
    ...
    this._params = (this._jsdoc["@params"] || []).map(item => new apiviewer.dao.Param(item, this));
    var arr = this._jsdoc["@return"];
    if (arr && arr.length)
      this._return = new apiviewer.dao.Param(arr[0], this);
    var arr = this._jsdoc["@throws"];
    this._throws = (arr && arr.length) ? new apiviewer.dao.Param(arr[0], this) : [];
