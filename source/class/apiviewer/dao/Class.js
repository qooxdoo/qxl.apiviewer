...
qx.Class.define("apiviewer.dao.Class",
{
  ...
  construct : function(className)
  {
    ...
    this._package = apiviewer.dao.Package.getParentPackage(className);
  ...
  members : {
    ...
    load: function() {
      ...
      var url = apiviewer.ClassLoader.getBaseUri() + "/transpiled/" + this._className.replace(/\./g, '/') + ".json";
      return this._loadingPromise = apiviewer.RequestUtil.get(url)
        .then(content => {
    ...
    _initMeta: function(meta) {
      ...
      this._defer = meta.defer ? [ new apiviewer.dao.Method(meta.defer, this, "defer") ] : [];
      ...
      if (meta.statics) {
        for (let name in meta.statics) {
          ...
          if (data.type == "variable") {
            this._constants.push(new apiviewer.dao.Constant(data, this, name));
          } else {
            this._staticMethods.push(new apiviewer.dao.Method(data, this, name));
          }
        }
      }
      ...
      if (meta.members) {
        for (let name in meta.members) {
          ...
          if (data.type == "function") {
            let obj = new apiviewer.dao.Method(data, this, name);
            if (data.mixin)
      ...
      if (meta.properties) {
        for (let name in meta.properties) {
          ...
          let obj = new apiviewer.dao.Property(data, this, name);
          if (data.mixin)
      ...
      if (meta.events) {
        for (let name in meta.events) {
          ...
          let obj = new apiviewer.dao.Event(data, this);
          if (data.mixin)
      ...
      function findClasses(tmp) {
        return apiviewer.dao.Class.findClasses(tmp).forEach(item => all.push(item));
      }
     ...
     * 
     * @return {qx.Promise(apiviewer.dao.Class[])} A list of direct child classes of the
     *         class.
     */
    getChildClasses : function() {
      if (!this._childClassesPromise) {
        if (this._meta.type == "class")
          this._childClassesPromise = apiviewer.dao.Class.findClasses(this._meta.descendants);
        else
     ...
     * 
     * @return {apiviewer.dao.Class[]} All interfaces declared at the class
     *         declaration.
     */
     ...
     * 
     * @return {apiviewer.dao.Class[]} All mixins declared at the class
     *         declaration.
     */
    getMixins : function() {
      return this._mixins; 
     ...
     * 
     * @return {qx.Promise(apiviewer.dao.Class[])} All classes including this mixin.
     */
    getIncluder : function() {
      if (!this._includersPromise) {
        if (this._meta.type == "mixin")
          this._includersPromise = apiviewer.dao.Class.findClasses(this._meta.descendants);
        else
     ...
     * 
     * @return {qx.Promise(apiviewer.dao.Class[])} All implementations of this interface.
     */
    getImplementations : function() {
      if (!this._implementationsPromise) {
        if (this._meta.type == "interface")
          this._implementationsPromise = apiviewer.dao.Class.findClasses(this._meta.descendants);
        else
     ...
     * 
     * @return {apiviewer.dao.Property} The named property
     */
     ...
     * @return {apiviewer.dao.Class[]} array of super classes of the given
     *         class.
     */
    getClassHierarchy : function(includeNativeObjects) {
      ...
      for (var currentClass = this; currentClass; currentClass = currentClass.getSuperClass()) {
        var isNative = apiviewer.dao.Class.isNativeObject(currentClass);
     ...
     * 
     * @return {apiviewer.dao.Class[]} array of super interfaces of the given
     *         interface.
     */
    ...
    loadDependedClasses: function() {
      return apiviewer.ClassLoader.loadClassList(this.getDependedClasses());
    },
  ...
  statics : {
    ...
    getClassByName : function(className, create) {
      var nativeClass = apiviewer.dao.Class._native_classes[className];
      if (nativeClass !== undefined)
        return nativeClass;
      
      var pkg = apiviewer.dao.Package.getParentPackage(className);
      if (!pkg) {
      ...
      if (!cls && create) {
        cls = new apiviewer.dao.Class(className);
      }
      return cls;
    },
    ...
    getClassesByName : function(classNames, create) {
      ...
      var result = classNames.map(name => apiviewer.dao.Class.getClassByName(name, create));
      return result;
    },
    ...
    findClasses: function(name) {
      ...
      var all = qx.lang.Array.toNativeArray(name)
        .filter(name => !apiviewer.dao.Class._native_classes[name])
        .map(name => apiviewer.dao.Class.getClassByName(name).load());
      return qx.Promise.all(all);
    },
    
    
    /**
     * Checks if the Class is a qooxdoo apiviewer.dao.Class Object or a native
     * one
     * 
     * @param clazz
     *          {apiviewer.dao.Class} the object to be checked
