...
qx.Class.define("apiviewer.ui.panels.InfoPanel", {
  ...
  members: {
     ...
     * @abstract
     * @param node
     *          {apiviewer.dao.ClassItem} the doc node of the item.
     * @param currentClassDocNode
     *          {apiviewer.dao.Class} the doc node of the currently displayed
     *          class
    ...
    getItemHtml: function(node, currentDocNode, showDetails) {
      ...
      if (this.itemHasDetails(node, currentDocNode)) {
        ...
        html.add('<img src="', qx.util.ResourceManager.getInstance().toUri("apiviewer/image/open.gif"), '" onclick="', this
    ...
    getPanelHtml: function(viewer) {
      ...
      html.add('<img class="openclose" src="', qx.util.ResourceManager.getInstance().toUri(
          'apiviewer/image/' + (this.getIsOpen() ? 'close.gif' : 'open.gif')), '" onclick="', this.__encodeObject(viewer),
     ...
     * @param currentClassDocNode
     *          {apiviewer.dao.Class} the currently displayed class
     * @return {apiviewer.dao.ClassItem[]} list of all items to display in the
     *         panel
     */
     ...
     * 
     * @param nodeArr
     *          {apiviewer.dao.ClassItem[]} array of class items
     * @param showProtected
     ...
     * 
     * @param classViewer
     *          {apiviewer.ui.ClassViewer} parent class viewer widget.
     * @param currentClassDocNode
     *          {apiviewer.dao.Class} the currently displayed class
     * @return {qx.Promise}
     */
    ...
    toggleShowItemDetails: function(itemName, fromClassName) {
      try {
        ...
        var fromClassNode = fromClassName ? apiviewer.dao.Class.getClassByName(fromClassName) : this.getDocNode();
        var node = null;
        ...
        opencloseImgElem.src = qx.util.ResourceManager.getInstance().toUri(
            showDetails ? 'apiviewer/image/close.gif' : 'apiviewer/image/open.gif');
  ...
  statics: {
     ...
     *          {apiviewer.dao.Class?null} the doc node of the class to use for
     ...
     *          text")
     * @param packageBaseClass
     *          {apiviewer.dao.Class?null} the doc node of the class to use when
    ...
    createItemLinkHtml: function(linkText, packageBaseClass, useIcon, useShortName) {
      ...
      var linkHtml = [ '<span style="white-space: nowrap;">', (typeof iconCode != "undefined" ? iconCode : ""),
          ...
          '" onmouseup="apiviewer.TabViewController.instance.onSelectItem(\'', fullItemName, '\'); return false;"', ' title="',
     ...
     * 
     * @param node
     *          {apiviewer.dao.ClassItem} item to get the information from
     * @param currentClassDocNode
     *          {apiviewer.dao.Class} the current class shown in the class
     *          viewer
     * @return {String} HTML fragment
     */
    createInheritedFromHtml: function(node, currentClassDocNode) {
      if (node.getClass().getType() != "mixin" && node.getClass() != currentClassDocNode) {
        var html = new qx.util.StringBuilder('<div class="item-detail-headline">', "Inherited from:", '</div>',
            '<div class="item-detail-text">', apiviewer.ui.panels.InfoPanel.createItemLinkHtml(node.getClass().getFullName() + "#"
                + node.getName()), '</div>');
        return html.get();
      } else {
        return "";
     ...
     * 
     * @param node
     *          {apiviewer.dao.ClassItem} item to get the the information from
     * @return {String} HTML fragment
     */
    createOverwriddenFromHtml: function(node) {
      if (node.getOverriddenFrom()) {
        var html = new qx.util.StringBuilder('<div class="item-detail-headline">', "Overrides:", '</div>',
            '<div class="item-detail-text">', apiviewer.ui.panels.InfoPanel.createItemLinkHtml(node.getOverriddenFrom()
                .getFullName()
                + "#" + node.getName()), '</div>');
        return html.get();
      } else {
     ...
     *          {String} the name of the item.
     * @param baseClassNode
     *          {apiviewer.dao.Class} class doc node
     ...
     * 
     * @param node
     *          {apiviewer.dao.Node} the doc node of the item.
     * @param packageBaseClass
     *          {apiviewer.dao.Class|apiviewer.dao.Package?null} the doc node of
     ...
     * 
     * @param typeNode
     *          {apiviewer.dao.ClassItem} the doc node to show the type for.
     * @param defaultType
        ...
        for (var j = 0; j < types.length; j++) {
          ...
          if (apiviewer.ui.ClassViewer.PRIMITIVES[typeName]) {
            if (apiviewer.ui.ClassViewer.MDC_LINKS[typeName]) {
              typeHtml.add('<span style="white-space: nowrap;"><a href="' + apiviewer.ui.ClassViewer.MDC_LINKS[typeName]
    ...
    createInfoRequiredByHtml: function(node) {
      ...
      if (requiredBy.length > 0) {
        ...
        for (var i = 0; i < requiredBy.length; i++) {
          html.add('<div class="item-detail-text">', apiviewer.ui.panels.InfoPanel.createItemLinkHtml(requiredBy[i].getFullName()
              + "#" + node.getName()), '</div>');
        }
      }
      return html.get();
