 ...
 * {@link Viewer}.
 */
qx.Class.define("qx.app.apiviewer.Controller",
{
  ...
  construct : function(widgetRegistry)
  {
    ...
    this._widgetRegistry = qx.app.apiviewer.MWidgetRegistry;

    this._titlePrefix = "API Documentation";
    document.title = this._titlePrefix;

    qx.app.apiviewer.ClassLoader.setBaseUri("..");

    this._detailLoader = this._widgetRegistry.getWidgetById("detail_loader");
    this._tabViewController = new qx.app.apiviewer.TabViewController(this._widgetRegistry);
  ...
  members :
  {
    ...
    load : function(url) {
      ...
      qx.app.apiviewer.RequestUtil.get(url)
        .then(content => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              if (state) {
                ...
                /*
                if (depth < qx.core.Environment.get("qx.app.apiviewer.initialTreeDepth") && packageDoc.getPackages().length > 0) {
    ...
    __syncMenuButton : function()
    {
      ...
      {
        menuButton.setIcon('qx/app/apiviewer/image/inherited_and_mixins_included.gif');
      }
      if(showInherited && !showMixins)
      {
        menuButton.setIcon('qx/app/apiviewer/image/method_public_inherited18.gif');
      }
      if(!showInherited && showMixins)
      {
        menuButton.setIcon('qx/app/apiviewer/image/overlay_mixin18.gif');
      }
      if(!showInherited && !showMixins)
      {
        menuButton.setIcon('qx/app/apiviewer/image/includes.gif');
      }
     ...
     * 
     * @param docTree
     *          {qx.app.apiviewer.dao.Package} root node of the documentation tree
     */
    __setDocTree : function(docTree)
    {
      ...
      classes.forEach(async (classname) => {
        let cls = qx.app.apiviewer.dao.Class.getClassByName(classname, true);
        await cls.load();
      ...
      var rootPackage = qx.app.apiviewer.dao.Package.getPackage(null);
      var end = new Date();
     ...
     * 
     * @param classNode
     *          {qx.app.apiviewer.dao.Class} class node to display
     */
    _selectClass : function(classNode, callback, self) {
      ...
      return classNode.loadDependedClasses()
        .then(() => {
          if (classNode instanceof qx.app.apiviewer.dao.Class) {
    ...
    __selectItem : function(fullItemName) {
      qx.app.apiviewer.LoadingIndicator.getInstance().show();
      ...
      this._tree.selectTreeNodeByClassName(className)
        .then(couldSelectTreeNode => {
          ...
          if (!couldSelectTreeNode) {
            ...
            qx.app.apiviewer.LoadingIndicator.getInstance().hide();
            return;
          }
          ...
          this._selectClass(qx.app.apiviewer.ClassLoader.getClassOrPackage(nodeName), () => {
            if (itemName) {
