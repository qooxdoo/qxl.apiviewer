 ...
 */
qx.Class.define("qx.app.apiviewer.ui.panels.InfoPanel", {
  ...
  construct: function(labelText, icon) {
    ...
    this._icon = icon;
    qx.app.apiviewer.ObjectRegistry.register(this);
  },
  ...
  members: {
     ...
     * @abstract
     * @param node
     *          {qx.app.apiviewer.dao.ClassItem} the doc node of the item.
     * @param currentClassDocNode
     *          {qx.app.apiviewer.dao.Class} the doc node of the currently displayed
     *          class
    ...
    getItemHtml: function(node, currentDocNode, showDetails) {
      if ((node instanceof qx.app.apiviewer.dao.Class) || node instanceof qx.app.apiviewer.dao.Package) {
      ...
      var iconUrl = qx.app.apiviewer.TreeUtil.getIconUrl(node, inherited);

      // Create the title row
      html.add('<tr class="', qx.app.apiviewer.ui.panels.InfoPanel.getItemCssClasses(node), '">');
      var tooltipText = this.getItemTooltip(node, currentDocNode);
      var tooltip = tooltipText ? 'title="' + tooltipText + '" alt="' + tooltipText + '"' : '';
      html.add('<td class="icon" ', tooltip, '>', qx.app.apiviewer.ui.ClassViewer.createImageHtml(iconUrl), '</td>');
      ...
      if (this.itemHasDetails(node, currentDocNode)) {
        ...
        html.add('<img src="', qx.util.ResourceManager.getInstance().toUri("qx/app/apiviewer/image/open.gif"), '" onclick="', this
    ...
    __encodeObject: function(object) {
      return "qx.app.apiviewer.ObjectRegistry.getObjectFromHashCode('" + object.toHashCode() + "')";
    },
    ...
    getPanelHtml: function(viewer) {
      ...
      html.add('<img class="openclose" src="', qx.util.ResourceManager.getInstance().toUri(
          'qx/app/apiviewer/image/' + (this.getIsOpen() ? 'close.gif' : 'open.gif')), '" onclick="', this.__encodeObject(viewer),
     ...
     * @param currentClassDocNode
     *          {qx.app.apiviewer.dao.Class} the currently displayed class
     * @return {qx.app.apiviewer.dao.ClassItem[]} list of all items to display in the
     *         panel
     */
     ...
     * 
     * @param nodeArr
     *          {qx.app.apiviewer.dao.ClassItem[]} array of class items
     * @param showProtected
     ...
     * @return {qx.app.apiviewer.dao.ClassItem[]} filtered list of items
     */
    ...
    _displayNodes: function(nodes, currentClassDocNode) {
      ...
      if (nodes && nodes.length > 0) {
        ...
        qx.app.apiviewer.ui.AbstractViewer.fixLinks(this.getBodyElement());
        qx.app.apiviewer.ui.AbstractViewer.highlightCode(this.getBodyElement());
     ...
     * 
     * @param classViewer
     *          {qx.app.apiviewer.ui.ClassViewer} parent class viewer widget.
     * @param currentClassDocNode
     *          {qx.app.apiviewer.dao.Class} the currently displayed class
     * @return {qx.Promise}
     */
    ...
    toggleShowItemDetails: function(itemName, fromClassName) {
      try {
        ...
        var fromClassNode = fromClassName ? qx.app.apiviewer.dao.Class.getClassByName(fromClassName) : this.getDocNode();
        var node = null;
        ...
        opencloseImgElem.src = qx.util.ResourceManager.getInstance().toUri(
            showDetails ? 'qx/app/apiviewer/image/close.gif' : 'qx/app/apiviewer/image/open.gif');

        // Update content
        textDiv.innerHTML = this.getItemTextHtml(node, this.getDocNode(), showDetails);
        this._postProcessLinks(textDiv);
        qx.app.apiviewer.ui.AbstractViewer.fixLinks(textDiv);
        qx.app.apiviewer.ui.AbstractViewer.highlightCode(textDiv);
      } catch (exc) {
  ...
  statics: {
     ...
     *          {qx.app.apiviewer.dao.Class?null} the doc node of the class to use for
     ...
     *          text")
     * @param packageBaseClass
     *          {qx.app.apiviewer.dao.Class?null} the doc node of the class to use when
    ...
    createItemLinkHtml: function(linkText, packageBaseClass, useIcon, useShortName) {
      ...
      if (typeof linkText == "string") {
        ...
        if (className == null || className.length == 0) {
          ...
          classNode = qx.app.apiviewer.dao.Class.getClassByName(className);
          ...
          if (!classNode || !classNode.classname || classNode.getPackage().getName() !== "") {
            ...
            // current class
            if (packageBaseClass instanceof qx.app.apiviewer.dao.Package) {
        ...
        if (useIcon) {
          ...
          if (classNode && classNode.isLoaded()) {
            ...
            if (itemName) {
              ...
              if (!itemNode && qx.app.apiviewer.UiModel.getInstance().getShowIncluded()) {
                if (qx.app.apiviewer.UiModel.getInstance().getShowInherited()) {
            ...
            if (itemNode && !(itemNode instanceof qx.app.apiviewer.dao.Package)) {
        ...
        if (itemNode instanceof qx.app.apiviewer.dao.Class) {
          className = label = itemNode.getFullName();
          
        } else if (itemNode instanceof qx.app.apiviewer.dao.ClassItem) {
      ...
      if (itemNode) {
        var iconUrl = qx.app.apiviewer.TreeUtil.getIconUrl(itemNode);
        var iconCode = qx.app.apiviewer.ui.ClassViewer.createImageHtml(iconUrl);
      ...
      var linkHtml = [ '<span style="white-space: nowrap;">', (typeof iconCode != "undefined" ? iconCode : ""),
          ...
          '" onmouseup="qx.app.apiviewer.TabViewController.instance.onSelectItem(\'', fullItemName, '\'); return false;"', ' title="',
     ...
     * 
     * @param node
     *          {qx.app.apiviewer.dao.ClassItem} item to get the information from
     * @param currentClassDocNode
     *          {qx.app.apiviewer.dao.Class} the current class shown in the class
     *          viewer
     * @return {String} HTML fragment
     */
    createInheritedFromHtml: function(node, currentClassDocNode) {
      if (node.getClass().getType() != "mixin" && node.getClass() != currentClassDocNode) {
        var html = new qx.util.StringBuilder('<div class="item-detail-headline">', "Inherited from:", '</div>',
            '<div class="item-detail-text">', qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(node.getClass().getFullName() + "#"
                + node.getName()), '</div>');
        return html.get();
      } else {
        return "";
     ...
     * 
     * @param node
     *          {qx.app.apiviewer.dao.ClassItem} item to get the the information from
     * @return {String} HTML fragment
     */
    createOverwriddenFromHtml: function(node) {
      if (node.getOverriddenFrom()) {
        var html = new qx.util.StringBuilder('<div class="item-detail-headline">', "Overrides:", '</div>',
            '<div class="item-detail-text">', qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(node.getOverriddenFrom()
                .getFullName()
                + "#" + node.getName()), '</div>');
        return html.get();
      } else {
     ...
     *          {String} the name of the item.
     * @param baseClassNode
     *          {qx.app.apiviewer.dao.Class} class doc node
     * @return {qx.app.apiviewer.dao.ClassItem} the classItem
     * 
     */
     ...
     * 
     * @param node
     *          {qx.app.apiviewer.dao.Node} the doc node of the item.
     * @param packageBaseClass
     *          {qx.app.apiviewer.dao.Class|qx.app.apiviewer.dao.Package?null} the doc node of
     ...
     * 
     * @param typeNode
     *          {qx.app.apiviewer.dao.ClassItem} the doc node to show the type for.
     * @param defaultType
    ...
    createTypeHtml: function(typeNode, defaultType, useShortName) {
      ...
      if (types.length == 0) {
        ...
        for (var j = 0; j < types.length; j++) {
          ...
          if (qx.app.apiviewer.ui.ClassViewer.PRIMITIVES[typeName]) {
            if (qx.app.apiviewer.ui.ClassViewer.MDC_LINKS[typeName]) {
              typeHtml.add('<span style="white-space: nowrap;"><a href="' + qx.app.apiviewer.ui.ClassViewer.MDC_LINKS[typeName]
            ...
            typeHtml.add(qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(linkText, typeNode.getClass(), false, true));
          }
    ...
    createInfoRequiredByHtml: function(node) {
      ...
      if (requiredBy.length > 0) {
        ...
        for (var i = 0; i < requiredBy.length; i++) {
          html.add('<div class="item-detail-text">', qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(requiredBy[i].getFullName()
              + "#" + node.getName()), '</div>');
        }
      }
      return html.get();
    ...
    createSourceLinkHtml: function(node) {
      ...
      var sourceUri = qx.app.apiviewer.ui.ClassViewer.getSourceUri(node);
      if (!sourceUri) {
        return "";
      }

      var title;
      if (node instanceof qx.app.apiviewer.dao.Class) {
