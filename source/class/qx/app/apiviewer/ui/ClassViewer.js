 ...
 */
qx.Class.define("qx.app.apiviewer.ui.ClassViewer",
{
  ...
  */
  ...
  {
    this.base(arguments);
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.ConstructorPanel("constructor"));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.EventPanel("events", true, true));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.StaticMethodsPanel("static methods"));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.ConstantPanel("constants", false, true));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.PropertyPanel("properties", true, true));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.MethodPanel("methods"));
    this.addInfoPanel(new qx.app.apiviewer.ui.panels.ChildControlsPanel("child controls"));
  ...
  */
  ...
  statics :
  {
     ...
     * Functions will be called with the current @link{qx.app.apiviewer.dao.Node} as the
    ...
    **/
    ...
    createImageHtml : function(imgUrl, tooltip, styleAttributes)
    {
      ...
      else
      {
        ...
        return qx.app.apiviewer.ui.ClassViewer.createOverlayImageHtml(18, 18, imgUrl, tooltip, styleAttributes);
      }
    },
     ...
     * map.
     *
     * @param node {qx.app.apiviewer.dao.Node} the documentation node for the title
  ...
  */
  ...
  members :
  {
     ...
     *
     * @param classNode {qx.app.apiviewer.dao.Class} the class documentation node for the title
    ...
    _getTitleHtml : function(classNode)
    {
      ...
      titleHtml.add(qx.app.apiviewer.ui.panels.InfoPanel.setTitleClass(classNode, className));
    ...
    _getTocHtml : function(classNode)
    {
      ...
      this.getPanels().forEach(panel => {
        ...
        tocItem.innerHTML = qx.app.apiviewer.ui.ClassViewer.createImageHtml(panel.getPanelIcon(), panel.getPanelTitle())+' ';
        ...
        if(panel instanceof qx.app.apiviewer.ui.panels.StaticMethodsPanel && qx.core.Environment.get("engine.name") == 'webkit') {
    ...
    _getDescriptionHtml : function(classNode)
    {
      ...
      var seeAlso = qx.app.apiviewer.ui.panels.InfoPanel.createSeeAlsoHtml(classNode);
      if (seeAlso) {
      ...
      if (classNode.getErrors().length > 0) {
        classHtml.add(
          '<div class="class-description">',
          qx.app.apiviewer.ui.panels.InfoPanel.createErrorHtml(classNode, classNode),
          '</div>'
        );
      }
     ...
     *
     * @param dependentClasses {qx.app.apiviewer.dao.Class[]} array of dependent classes
    ...
    __getDependentClassesHtml : function(dependentClasses, title)
    {
      ...
      if (dependentClasses.length > 0)
      {
        ...
        for (var i=0; i<dependentClasses.length; i++)
        {
          ...
          result.add(qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(dependentClasses[i], null, true, false));
        }
     ...
     *
     * @param classNode {qx.app.apiviewer.dao.Class} class node
     * @return {String} HTML fragemnt
     */
    __getClassHierarchyHtml: function(classNode)
    {
      var ClassViewer = qx.app.apiviewer.ui.ClassViewer;
      ...
      classHtml.add(ClassViewer.createImageHtml("qx/app/apiviewer/image/class18.gif"), '<span style="white-space: nowrap;"><a href="https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Object" target="_blank" title="Object">Object</a></span>');
      var indent = 0;

      for (var i=classHierarchy.length-1; i>=0; i--)
      {
        if(qx.app.apiviewer.dao.Class.isNativeObject(classHierarchy[i]) && classHierarchy[i] === Object) {
          continue;
        }
        classHtml.add('<div>');
        classHtml.add(
          ClassViewer.createImageHtml("qx/app/apiviewer/image/nextlevel.gif", null, "margin-left:" + indent + "px"),
          !qx.app.apiviewer.dao.Class.isNativeObject(classHierarchy[i]) ? ClassViewer.createImageHtml(qx.app.apiviewer.TreeUtil.getIconUrl(classHierarchy[i])) : ClassViewer.createImageHtml("qx/app/apiviewer/image/class18.gif")
        );

        if (i !== 0) {
          if(!qx.app.apiviewer.dao.Class.isNativeObject(classHierarchy[i]))
          {
            classHtml.add(qx.app.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(classHierarchy[i].getFullName(), null, false));
          }
          else
          {
            ...
            classHtml.add('<span style="white-space: nowrap;"><a href="'+qx.app.apiviewer.ui.ClassViewer.MDC_LINKS[name]+'" target="_blank" title="'+name+'">'+name+'</a></span>');
          }
        } else {
    ...
    {
      var ClassViewer = qx.app.apiviewer.ui.ClassViewer;
      var TreeUtil = qx.app.apiviewer.TreeUtil;
      var InfoPanel = qx.app.apiviewer.ui.panels.InfoPanel;
      ...
      for (var i=hierarchy.length - 1; i >= 0; i--) {
        ...
        if (hierarchy[i].getSuperInterfaces().length > 0) {
          html.add(ClassViewer.createImageHtml("qx/app/apiviewer/image/nextlevel.gif", null, "margin-left:" + indent + "px"));
    ...
    showItem : function(itemName)
    {
      ...
      if (this._markedElement) {
        this._markedElement.className = qx.app.apiviewer.ui.panels.InfoPanel.getItemCssClasses(this._markedItemNode);
      }
    ...
    {
       var uiModel = qx.app.apiviewer.UiModel.getInstance();
     ...
     *
     * @param itemNode {qx.app.apiviewer.dao.Class} the doc node of the item.
