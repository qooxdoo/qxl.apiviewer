...
qx.Class.define("qx.app.apiviewer.dao.Package", {
  ...
  construct: function(packageName) {
    ...
    this._packages = {};
    if (packageName) {
      this._parentPackage = qx.app.apiviewer.dao.Package.getParentPackage(packageName);
  ...
  members: {
    ...
    loadDependedClasses: function() {
      return qx.app.apiviewer.ClassLoader.loadClassList(this.getClasses());
    },
  ...
  statics: {
    ...
    getPackage: function(name, create) {
      var root = qx.app.apiviewer.dao.Package.__rootPackage;
      if (!root) {
        root = qx.app.apiviewer.dao.Package.__rootPackage = new qx.app.apiviewer.dao.Package("");
      }
      if (!name) {
        return root;
      }
      ...
      for (var i = 0; i < segs.length; i++) {
        ...
        if (!tmp) {
          if (!create)
            return null;
          tmp = new qx.app.apiviewer.dao.Package(i == 0 ? segs[i] : current.getFullName() + "." + segs[i]);
        }
        current = tmp;
        parentName += segs[i] + ".";
      }
    ...
    getParentPackage: function(name) {
      ...
      if (pos < 0) {
        return qx.app.apiviewer.dao.Package.__rootPackage;
      }
      var parentName = name.substring(0, pos);
      return qx.app.apiviewer.dao.Package.getPackage(parentName, true);
    }
  }
});
