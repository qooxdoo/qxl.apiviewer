  ...
  members: {
    ...
    getItemTitleHtml(method) {
      ...
      var titleHtml = new qx.util.StringBuilder(
        qxl.apiviewer.ui.panels.InfoPanel.setTitleClass(method, title)
      );

      // Add the title (the method signature)
      titleHtml.add(
        '<span class="method-signature"><span class="parenthesis">(</span>'
      );
      ...
      for (var i = 0; i < params.length; i++) {
        ...
        titleHtml.add(
          ...
          qxl.apiviewer.ui.panels.InfoPanel.createTypeHtml(param, "var"),
          "</span> <code>",
          param.getName(),
          "</code>"
        );
    ...
    getItemTypeHtml(method) {
      ...
      if (!method.isConstructor()) {
        typeHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createTypeHtml(
            method.getReturn(),
            "void"
          )
        );
     ...
     */
    getItemTextHtml(method, currentClassDocNode, showDetails) {
      ...
      if (showDetails) {
        ...
        if (params.length > 0) {
          ...
          for (let i = 0; i < params.length; i++) {
            ...
            */
            ...
            if (defaultValue) {
              textHtml.add(
                " (" + (param.isOptional() ? "optional; " : "") + "default: ",
                defaultValue,
                ") "
              );
        ...
        if (applyToProperties && applyToProperties.length > 0) {
          ...
          textHtml.add(
            '<div class="item-detail-headline">',
            applyToProperties.length == 1
              ? "Apply method of property:"
              : "Apply method of properties:",
            "</div>",
            '<div class="item-detail-text">'
          );
        ...
        if (throwsEntries.length > 0) {
          ...
          for (let i = 0; i < throwsEntries.length; i++) {
            ...
            var throwsEntryType = throwsEntry.getType()
              ? throwsEntry.getType()
              : throwsEntry.getDefaultType();

            textHtml.add('<div class="item-detail-text">');

            textHtml.add(
              '<span class="parameter-type">',
              throwsEntryType === throwsEntry.getDefaultType()
                ? throwsEntry.getDefaultType()
                : qxl.apiviewer.ui.panels.InfoPanel.createItemLinkHtml(
                    throwsEntryType
                  ),
        ...
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createAccessHtml(method)
        );
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createIncludedFromHtml(
            method,
            currentClassDocNode
          )
        );
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createOverwriddenFromHtml(method)
        );
        textHtml.add(
        ...
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createInfoRequiredByHtml(method)
        );
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createSeeAlsoHtml(method)
        );
        textHtml.add(
        ...
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createDeprecationHtml(
            method,
            "function"
          )
        );
        textHtml.add(
          qxl.apiviewer.ui.panels.InfoPanel.createSourceLinkHtml(method)
        );
      }
